...
export function useHandCricket() {
  ...
  /**
   * PRO MODE entry point: applies a pre-resolved ball result from GameplayEngine.
   * Mirrors playBall's state-mutation logic but uses the supplied runs/desc/aiMove
   * directly instead of re-rolling internally. Does NOT touch playBall.
   */
  const applyBallResult = useCallback(
    (userMove: Move, aiMove: AiMove, engineRuns: number | "OUT", desc: string) => {
      setGame((prev) => {
        if (prev.phase === "not_started" || prev.phase === "finished") return prev;

        const ballResult: BallResult = { userMove, aiMove, runs: engineRuns, description: desc };
        const newHistory = [...prev.ballHistory, ballResult];
        ...
        const currentBalls =
          prev.currentInnings === 1 ? newState.innings1Balls : newState.innings2Balls;
        ...
        if (prev.isBatting) {
          if (engineRuns === "OUT") {
            ...
            const newScore = prev.userScore + (engineRuns as number);
            newState.userScore = newScore;
            if (prev.target && newScore >= prev.target) {
              newState.phase = "finished";
              newState.result = "win";
          ...
          // Bowling: AI is batting. engineRuns is from AI's perspective (positive = AI scored).
          if (engineRuns === "OUT") {
            const newAiWickets = prev.aiWickets + 1;
            newState.aiWickets = newAiWickets;
            if (isWicketsExhausted(newAiWickets, prev.config)) {
            ...
            const aiRuns = Math.abs(engineRuns as number);
            const newAiScore = prev.aiScore + aiRuns;
            newState.aiScore = newAiScore;
            if (prev.target && newAiScore >= prev.target) {
              newState.phase = "finished";
        ...
        // Overs exhaustion check (mirrors playBall)
        if (newState.phase !== "finished" && newState.phase !== "not_started") {
          if (isOversExhausted(currentBalls, prev.config)) {
            if (prev.currentInnings === 1) {
              if (prev.isBatting) {
              ...
              newState = {
                ...
                phase: "finished",
                result:
                  userTotal > aiTotal ? "win" : userTotal < aiTotal ? "loss" : "draw",
              };
            }
          }
        }
  ...
  return { game, startGame, playBall, applyBallResult, resetGame };
}
