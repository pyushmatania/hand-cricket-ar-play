...
export default function WorldCupScreen({ onHome }: Props) {
  ...
  const pickTeam = async (team: typeof TEAMS[0]) => {
    ...
    const group: GroupTeam[] = [{ ...team, points: 0, wins: 0, losses: 0 }, ...others.map(t => ({ ...t, points: 0, wins: 0, losses: 0 }))];
    setGroupTeams(group);
    const id = await createTournament({ format: "worldcup", name: `World Cup - ${team.name}`, placement: null, metadata: { team: team.name, groupTeams: group.map(t => t.name) } });
    setTournamentId(id);
    setPhase("group");
  };
  ...
  const resetMatch = () => { setScore(0); setOppScore(0); setBalls(0); setInnings(1); setTarget(0); setMatchResult(null); setLastBall(""); ballsRef.current = 0; };
  ...
  const playBall = useCallback((move: number) => {
    ...
    if (innings === 1) {
      ...
      if (move === aiMove) { setLastBall("WICKET!"); finishMatch("win"); return; }
  ...
  const persistFixture = (roundNum: number, oppName: string, myS: number, oppS: number, won: boolean) => {
    ...
    saveFixture({ tournamentId, roundNumber: roundNum, matchIndex: idx, playerAId: user.id, playerBId: null, playerAScore: myS, playerBScore: oppS, winnerId: won ? user.id : null, status: "completed" });
  };
  ...
  const handleMatchDone = () => {
    ...
    const roundNum = knockoutOpponents.length > 0 ? (knockoutStage === "super8" ? 2 : knockoutStage === "semi" ? 3 : 4) : 1;
    ...
    if (phase === "match" && knockoutStage === "super8" && knockoutOpponents.length === 0) {
      ...
      const updatedTeams = groupTeams.map(t => {
        if (t.name === myTeam.name) { const w = t.wins + (matchResult === "win" ? 1 : 0); const l = t.losses + (matchResult === "loss" ? 1 : 0); return { ...t, points: w * 2, wins: w, losses: l }; }
        if (t.name === matchOpponent.name) { const w = t.wins + (matchResult === "loss" ? 1 : 0); const l = t.losses + (matchResult === "win" ? 1 : 0); return { ...t, points: w * 2, wins: w, losses: l }; }
        const won = Math.random() > 0.5; const w = t.wins + (won ? 1 : 0); const l = t.losses + (won ? 0 : 1); return { ...t, points: w * 2, wins: w, losses: l };
      });
      setGroupTeams(updatedTeams);
      const opponents = updatedTeams.filter(t => t.name !== myTeam.name);
      if (nextIdx >= opponents.length) {
        const qualifiers = [...updatedTeams].sort((a, b) => b.points - a.points).slice(0, 4).filter(t => t.name !== myTeam.name);
        setKnockoutOpponents(qualifiers.slice(0, 2));
        setKnockoutIdx(0); setKnockoutStage("super8"); setPhase("super8");
      } else { setPhase("group"); }
    } else {
      ...
      if (matchResult === "loss") {
        ...
        setFinalPlacement(placements[knockoutStage] || "Eliminated"); setPhase("results"); return;
      }
      if (knockoutStage === "super8") {
        const nextKO = knockoutIdx + 1;
        if (nextKO >= knockoutOpponents.length) { setKnockoutStage("semi"); const semiOpp = TEAMS.filter(t => t.name !== myTeam.name).sort(() => Math.random() - 0.5)[0]; setKnockoutOpponents([{ ...semiOpp, points: 0, wins: 0, losses: 0 }]); setKnockoutIdx(0); setPhase("semi"); }
        else { setKnockoutIdx(nextKO); setPhase("super8"); }
      } else if (knockoutStage === "semi") { setKnockoutStage("final"); const finalOpp = TEAMS.filter(t => t.name !== myTeam.name).sort((a, b) => b.strength - a.strength)[0]; setKnockoutOpponents([{ ...finalOpp, points: 0, wins: 0, losses: 0 }]); setKnockoutIdx(0); setPhase("final"); }
      else { setFinalPlacement("🏆 WORLD CHAMPION"); setPhase("results"); }
    }
  };

  const startKnockoutMatch = () => { const opp = knockoutOpponents[knockoutIdx]; setMatchOpponent(opp); resetMatch(); setPhase("match"); };

  // PICK TEAM
  if (phase === "pick") {
    return (
      <div className="fixed inset-0 bg-background flex flex-col overflow-hidden">
        <div className="absolute inset-0 pointer-events-none" style={{ background: "radial-gradient(ellipse at 50% 20%, hsl(217 40% 12%) 0%, hsl(220 20% 6%) 70%)" }} />
        <div className="relative z-10 flex items-center justify-between px-4 pt-4 pb-2">
          ...
          <h2 className="font-display text-sm tracking-wider text-accent">🌍 WORLD CUP</h2>
          <div className="w-9" />
        </div>
        <div className="relative z-10 text-center py-4">
          <h3 className="font-display text-lg text-foreground">PICK YOUR NATION</h3>
          <p className="font-body text-[10px] text-muted-foreground">Group Stage → Super 8 → Semi → Final</p>
        </div>
          ...
          <div className="grid grid-cols-2 gap-2">
            {TEAMS.map(team => (
              <motion.button key={team.name} whileTap={{ scale: 0.95 }} onClick={() => pickTeam(team)}
                className="text-left p-3 rounded-xl stadium-glass">
                <span className="text-2xl">{team.flag}</span>
                <p className="font-display text-[10px] text-foreground mt-1">{team.name}</p>
  ...
  if (phase === "group" && myTeam) {
    ...
    return (
      <div className="fixed inset-0 bg-background flex flex-col overflow-hidden">
        <div className="absolute inset-0 pointer-events-none" style={{ background: "radial-gradient(ellipse at 50% 20%, hsl(217 30% 10%) 0%, hsl(220 20% 6%) 70%)" }} />
        <div className="relative z-10 flex items-center justify-between px-4 pt-4 pb-2">
          <motion.button whileTap={{ scale: 0.9 }} onClick={onHome} className="w-9 h-9 rounded-xl stadium-glass flex items-center justify-center text-sm text-foreground">←</motion.button>
          <h2 className="font-display text-[10px] tracking-widest text-accent">GROUP STAGE</h2>
          <div className="w-9" />
        </div>
        <div className="relative z-10 px-4 mt-2 mb-4">
          <div className="rounded-xl overflow-hidden scoreboard-metal">
            <div className="grid grid-cols-4 px-3 py-2 text-center" style={{ background: "hsl(220 15% 12%)" }}>
              <span className="font-display text-[8px] text-muted-foreground text-left">TEAM</span>
              <span className="font-display text-[8px] text-muted-foreground">W</span>
              <span className="font-display text-[8px] text-muted-foreground">L</span>
              <span className="font-display text-[8px] text-muted-foreground">PTS</span>
            </div>
            {[...groupTeams].sort((a, b) => b.points - a.points).map(t => (
              <div key={t.name} className={`grid grid-cols-4 px-3 py-1.5 text-center ${t.name === myTeam.name ? "bg-accent/10" : ""}`}>
                <span className="font-body text-[10px] text-foreground text-left">{t.flag} {t.name}</span>
                <span className="font-display text-[10px] text-foreground">{t.wins}</span>
                <span className="font-display text-[10px] text-foreground">{t.losses}</span>
                <span className="font-display text-[10px] text-secondary">{t.points}</span>
              </div>
            ))}
          </div>
        </div>
        {currentOpp && (
          <div className="relative z-10 flex-1 flex flex-col items-center justify-center px-4">
            <p className="font-display text-[9px] text-muted-foreground tracking-wider mb-4">MATCH {groupMatchIdx + 1} OF {opponents.length}</p>
            <div className="flex items-center gap-6 mb-6">
              <div className="text-center"><span className="text-3xl">{myTeam.flag}</span><p className="font-display text-[10px] text-foreground mt-1">{myTeam.name}</p></div>
              <span className="font-display text-lg text-muted-foreground">VS</span>
              <div className="text-center"><span className="text-3xl">{currentOpp.flag}</span><p className="font-display text-[10px] text-foreground mt-1">{currentOpp.name}</p></div>
            </div>
            <V10Button variant="primary" size="lg" glow onClick={() => startGroupMatch(groupMatchIdx)}>⚡ PLAY</V10Button>
          </div>
        )}
      </div>
    );
    ...
    return (
      <div className="fixed inset-0 bg-background flex flex-col items-center justify-center">
        <div className="absolute inset-0 pointer-events-none"
          style={{ background: `radial-gradient(ellipse at 50% 40%, ${phase === "final" ? "hsl(43 40% 12%)" : "hsl(0 30% 10%)"} 0%, hsl(220 20% 6%) 70%)` }} />
        <motion.div initial={{ scale: 0.5, opacity: 0 }} animate={{ scale: 1, opacity: 1 }} className="relative z-10 text-center">
          <p className="font-display text-[10px] tracking-[0.3em] text-muted-foreground mb-3">{stageNames[phase]}</p>
          <div className="flex items-center gap-6 mb-6">
            <div className="text-center"><span className="text-4xl">{myTeam.flag}</span><p className="font-display text-xs text-foreground mt-1">{myTeam.name}</p></div>
            <span className="font-display text-xl text-accent">VS</span>
            <div className="text-center"><span className="text-4xl">{opp?.flag}</span><p className="font-display text-xs text-foreground mt-1">{opp?.name}</p></div>
          </div>
          {allResults.length > 0 && (
          ...
          <V10Button variant={phase === "final" ? "gold" : "danger"} size="lg" glow onClick={startKnockoutMatch}>
            ⚡ PLAY MATCH
          </V10Button>
        </motion.div>
      </div>
  ...
  if (phase === "match" && matchOpponent && myTeam) {
    ...
    return (
      <div className="fixed inset-0 bg-background flex flex-col overflow-hidden">
        ...
        <div className="relative z-10 flex items-center justify-between px-4 pt-4 pb-2">
          <span className="font-display text-[9px] tracking-widest text-accent">{myTeam.flag} vs {matchOpponent.flag}</span>
        ...
        <div className="relative z-10 text-center py-4">
          <div className="flex items-center justify-center gap-6">
            <div><p className="font-display text-[9px] text-muted-foreground">{myTeam.name}</p><p className="font-display text-3xl text-foreground">{score}</p></div>
            <span className="font-display text-xs text-muted-foreground">vs</span>
            <div><p className="font-display text-[9px] text-muted-foreground">{matchOpponent.name}</p><p className="font-display text-3xl text-foreground">{oppScore}</p></div>
          </div>
    ...
    return (
      <div className="fixed inset-0 bg-background flex flex-col items-center justify-center">
        ...
        <motion.div initial={{ scale: 0.5, opacity: 0 }} animate={{ scale: 1, opacity: 1 }} className="relative z-10 text-center px-6">
          ...
          <p className="font-display text-[10px] tracking-[0.3em] text-muted-foreground mt-4 mb-2">WORLD CUP</p>
          ...
          <div className="flex gap-3">
            ...
            <V10Button variant="primary" size="md" glow onClick={() => {
              setPhase("pick"); setMyTeam(null); setGroupTeams([]); setGroupMatchIdx(0);
