...
export default function FriendStatsModal({ friend, onClose, onChallenge }: Props) {
  ...
  const [allFriendMatches, setAllFriendMatches] = useState<any[]>([]);
  ...
  useEffect(() => {
    ...
    setTab("overview");
    setLoading(true);
    // Functions are defined below but hoisted via useCallback
    const doLoad = async () => {
      const { data: myData } = await supabase.from("profiles").select("user_id, display_name, wins, losses, draws, total_matches, high_score, best_streak, current_streak, abandons, avatar_url, avatar_index, xp, coins, rank_tier, login_streak, best_login_streak, total_runs, total_sixes, total_fours").eq("user_id", user.id).single();
      if (myData) setMyProfile(myData as any);
      const { data: fpData } = await supabase.from("profiles").select("user_id, display_name, wins, losses, draws, total_matches, high_score, best_streak, current_streak, abandons, avatar_url, avatar_index, xp, coins, rank_tier, login_streak, best_login_streak, total_runs, total_sixes, total_fours").eq("user_id", friend.user_id).single();
      if (fpData) setFullFriendProfile(fpData as unknown as FriendProfile);
    };
          ...
          <div className="p-4" style={{ borderBottom: "1px solid hsl(220 15% 14%)" }}>
            ...
            <div className="flex items-center gap-2 mt-3">
              <div className="flex-1 concrete-card rounded-lg py-1.5 px-2 text-center">
                ...
                <span className="font-display text-sm font-bold text-primary">{fp.xp ?? 0}</span>
              </div>
              <div className="flex-1 concrete-card rounded-lg py-1.5 px-2 text-center">
                <span className="text-[6px] text-muted-foreground font-display tracking-widest block">COINS</span>
                <span className="font-display text-sm font-bold text-secondary">{fp.coins ?? 0}</span>
              </div>
              <div className="flex-1 concrete-card rounded-lg py-1.5 px-2 text-center">
                <span className="text-[6px] text-muted-foreground font-display tracking-widest block">STREAK</span>
                <span className="font-display text-sm font-bold text-score-gold">🔥 {fp.current_streak ?? 0}</span>
              </div>
            ...
            onClick={() => onChallenge(friend.user_id)}
                className="w-full mt-3 py-2.5 rounded-xl font-display text-[10px] font-bold tracking-widest text-white"
                style={{
              ...
              <AnimatePresence mode="wait">
                ...
                {tab === "overview" && (
                  <motion.div key="overview" initial={{ opacity: 0, x: -10 }} animate={{ opacity: 1, x: 0 }} exit={{ opacity: 0, x: 10 }} className="space-y-3">
                    ...
                    <div className="grid grid-cols-4 gap-2">
                      {[
                        ...
                        <div key={s.label} className="concrete-card rounded-xl p-2 text-center">
                          ...
                          <span className={`font-display text-lg ${s.color} block leading-none mt-1`}>{s.value}</span>
                    ...
                    <div className="grid grid-cols-3 gap-2">
                      {[
                        ...
                        <div key={s.label} className="concrete-card rounded-lg p-2 text-center">
                          ...
                          <span className="font-display text-sm font-bold text-foreground block leading-none mt-0.5">{s.value}</span>
                          <span className="text-[5px] font-display text-muted-foreground tracking-widest">{s.label}</span>
                        </div>
                      ))}
                    </div>
                    ...
                    {friendPvp && friendPvp.totalGames > 0 && (
                      <div className="concrete-card rounded-xl p-2.5 border border-primary/15">
                        ...
                        <div className="grid grid-cols-4 gap-1.5">
                          {[
                            ...
                            <div key={s.label} className="text-center">
                              <span className={`font-display text-sm font-bold ${s.color} block leading-none`}>{s.value}</span>
                              <span className="text-[5px] font-display text-muted-foreground tracking-widest">{s.label}</span>
                            </div>
                          ))}
                        </div>
                    ...
                    {/* Login streak */}
                    {(fp.login_streak ?? 0) > 0 && (
                      <div className="concrete-card rounded-xl p-2.5 flex items-center gap-3">
                        <span className="text-lg">📅</span>
                        <div className="flex-1">
                          <span className="text-[7px] text-muted-foreground font-display tracking-widest block">LOGIN STREAK</span>
                          <span className="font-display text-sm font-bold text-primary">{fp.login_streak} days</span>
                        </div>
                        ...
                        <div className="space-y-1.5">
                          {[
                            ...
                            <div key={row.label} className="flex items-center gap-2 concrete-card rounded-lg px-3 py-2">
                              <span className={`font-display text-xs font-bold flex-1 text-left ${row.mineNum > row.theirsNum ? "text-neon-green" : row.mineNum < row.theirsNum ? "text-out-red" : "text-foreground"}`}>
                                {row.mine}
                              </span>
                              <span className="text-[6px] font-display text-muted-foreground tracking-widest w-20 text-center">{row.label}</span>
                              <span className={`font-display text-xs font-bold flex-1 text-right ${row.theirsNum > row.mineNum ? "text-neon-green" : row.theirsNum < row.mineNum ? "text-out-red" : "text-foreground"}`}>
                                {row.theirs}
                              </span>
                            </div>
                          ))}
                ...
                {tab === "individual" && (
                  <motion.div key="individual" initial={{ opacity: 0, x: -10 }} animate={{ opacity: 1, x: 0 }} exit={{ opacity: 0, x: 10 }} className="space-y-3">
                    ...
                    {/* Overall record */}
                    <div className="concrete-card rounded-xl p-3">
                      ...
                      <div className="grid grid-cols-4 gap-2">
                        {[
                          ...
                          <div key={s.label} className="text-center">
                            <span className="text-sm block">{s.emoji}</span>
                            <span className={`font-display text-lg ${s.color} block leading-none mt-0.5`}>{s.value}</span>
                            <span className="text-[5px] font-display text-muted-foreground tracking-widest">{s.label}</span>
                          </div>
                        ))}
                      </div>
                    ...
                    <div className="grid grid-cols-3 gap-2">
                      <div className="concrete-card rounded-lg p-2 text-center">
                        ...
                        <span className="font-display text-sm font-bold text-foreground">{fp.draws ?? 0}</span>
                        <span className="text-[5px] font-display text-muted-foreground tracking-widest block">DRAWS</span>
                      </div>
                      <div className="concrete-card rounded-lg p-2 text-center">
                        <span className="text-xs block">🔥</span>
                        <span className="font-display text-sm font-bold text-secondary">{fp.current_streak ?? 0}</span>
                      ...
                      <div className="concrete-card rounded-lg p-2 text-center">
                        ...
                        <span className="font-display text-sm font-bold text-out-red">{fp.abandons ?? 0}</span>
                    ...
                    {recentFriendMatches.length > 0 && (
                      <div className="concrete-card rounded-xl p-3">
                        ...
                        <div className="flex items-center justify-between mt-2 pt-2 border-t border-muted/10">
                          ...
                          <span className="font-display text-sm font-bold text-score-gold">{fp.best_streak}</span>
                        </div>
                        <div className="flex items-center justify-between mt-1">
                          ...
                          <span className={`font-display text-sm font-bold ${(fp.current_streak ?? 0) > 0 ? "text-neon-green" : "text-foreground"}`}>
                        ...
                        <div className="space-y-1.5">
                          {[
                            ...
                            <div key={row.label} className="flex items-center justify-between py-1 border-b border-muted/10 last:border-0">
                              ...
                              <span className={`font-display text-sm font-bold ${row.color}`}>{row.value}</span>
                            </div>
                          ))}
                        </div>
                      </div>
                    ...
                    {/* Rank & Progression */}
                    <div className={`concrete-card rounded-xl p-3 border ${friendTier.borderColor}`}>
                      ...
                      <div className="relative z-10">
                        ...
                        <div className="flex items-center gap-3">
                          ...
                          <div>
                            <span className={`font-display text-sm font-bold ${friendTier.color}`}>{friendTier.name}</span>
                    ...
                    {(() => {
                      ...
                      return (
                        <div className="concrete-card rounded-xl p-3 border border-neon-green/15">
                          ...
                          <div className="flex items-center gap-3">
                            <div className="w-10 h-10 rounded-xl bg-neon-green/10 border border-neon-green/20 flex items-center justify-center">
                              <span className="font-display text-lg text-neon-green">{lastWin.user_score}</span>
                            </div>
                            <div className="flex-1">
                    ...
                    {/* Lifetime Stats */}
                    <div className="concrete-card rounded-xl p-3 border border-score-gold/15">
                      <div className="flex items-center gap-2 mb-2">
                        <span className="text-sm">📊</span>
                      ...
                      {friendMatchStats && (
                        <div className="grid grid-cols-2 gap-2 mt-2">
                          <div className="concrete-card rounded-lg p-2 text-center">
                            ...
                            <span className="font-display text-sm font-bold text-foreground">{friendMatchStats.avgScore}</span>
                          </div>
                          <div className="concrete-card rounded-lg p-2 text-center">
                            <span className="text-[5px] font-display text-muted-foreground tracking-widest block">STRIKE RATE</span>
                            <span className="font-display text-sm font-bold text-primary">{friendMatchStats.strikeRate}</span>
                          </div>
                        </div>
                      )}
                    </div>
                              ...
                              <div key={m.id} className={`p-2 rounded-xl ${
                                ...
                                <div className="flex items-center gap-2">
                                  ...
                                  <div className="flex items-center gap-1.5">
                                    <span className={`font-display text-xs font-bold ${won ? "text-neon-green" : "text-foreground"}`}>{m.user_score}</span>
                                    <span className="text-[6px] text-muted-foreground font-display">vs</span>
                                    <span className={`font-display text-xs font-bold ${lost ? "text-out-red" : "text-foreground"}`}>{m.ai_score}</span>
                                  </div>
                    ...
                    {myMatchStats && friendMatchStats ? (
                      <div className="space-y-2">
                        {[
                          ...
                          return (
                            <div key={row.label} className="flex items-center gap-2 concrete-card rounded-lg px-3 py-2">
                              <span className={`font-display text-xs font-bold flex-1 text-left ${mN > tN ? "text-neon-green" : mN < tN ? "text-out-red" : "text-foreground"}`}>
                                {row.mine}
                              </span>
                              ...
                              <span className={`font-display text-xs font-bold flex-1 text-right ${tN > mN ? "text-neon-green" : tN < mN ? "text-out-red" : "text-foreground"}`}>
                                {row.theirs}
                              </span>
                            </div>
                          );
                      ...
                      <div className="concrete-card rounded-xl p-6 text-center">
                ...
                {tab === "rivalry" && (
                  <motion.div key="rivalry" initial={{ opacity: 0, x: -10 }} animate={{ opacity: 1, x: 0 }} exit={{ opacity: 0, x: 10 }} className="space-y-3">
                    {h2h && h2h.totalGames > 0 ? (
                      <>
                        {/* Hero W/L record */}
                        <div className="concrete-card rounded-xl p-4 border border-primary/15">
                          ...
                          {[
                            ...
                            return (
                              <div key={s.label} className="flex items-center gap-2 concrete-card rounded-lg px-3 py-2">
                                <span className={`font-display text-xs font-bold flex-1 text-left ${mN > tN ? "text-neon-green" : mN < tN ? "text-out-red" : "text-foreground"}`}>{s.mine}</span>
                                <div className="flex items-center gap-1 w-24 justify-center">
                                ...
                                <span className={`font-display text-xs font-bold flex-1 text-right ${tN > mN ? "text-neon-green" : tN < mN ? "text-out-red" : "text-foreground"}`}>{s.theirs}</span>
                              </div>
                            );
                          })}
                          <div className="flex justify-between text-[6px] font-display text-muted-foreground tracking-widest px-1">
                        ...
                        <div className="grid grid-cols-2 gap-2">
                          <div className="concrete-card rounded-lg p-2.5 text-center">
                            ...
                            <span className={`font-display text-lg ${h2h.currentStreak > 0 ? "text-neon-green" : h2h.currentStreak < 0 ? "text-out-red" : "text-foreground"}`}>
                          ...
                          <div className="concrete-card rounded-lg p-2.5 text-center">
                            ...
                            <span className={`font-display text-lg ${h2h.bestStreak > 0 ? "text-neon-green" : "text-out-red"}`}>
                        ...
                        <div className="concrete-card rounded-xl p-3 border border-primary/10">
                          {(() => {
                            ...
                            return (
                              <div className="space-y-2">
                                <div className="flex items-center justify-between">
                                  ...
                                  <div className="flex items-center gap-1">
                                    <span className="font-display text-sm font-bold text-foreground">{myRecent5Avg}</span>
                                ...
                                <div className="flex items-center justify-between">
                                  ...
                                  <div className="flex items-center gap-1">
                                    <span className="font-display text-sm font-bold text-foreground">{theirRecent5Avg}</span>
                                ...
                                <div className="flex items-center justify-between">
                                  ...
                                  <span className="font-display text-xs font-bold text-primary">{myOverallAvg}</span>
                                </div>
                                <div className="flex items-center justify-between">
                                  ...
                                  <span className="font-display text-xs font-bold text-out-red">{theirOverallAvg}</span>
                                </div>
                              </div>
                            );
                          })()}
                            ...
                            return (
                              <div key={m.id} className={`p-2.5 rounded-xl ${
                                ...
                                <div className="flex items-center gap-2">
                                  ...
                                  <div className="flex items-center gap-1.5">
                                    <span className={`font-display text-sm font-bold ${won ? "text-neon-green" : "text-foreground"}`}>{myScore}</span>
                                    <span className="text-[6px] text-muted-foreground font-display">vs</span>
                                    <span className={`font-display text-sm font-bold ${lost ? "text-out-red" : "text-foreground"}`}>{theirScore}</span>
                                  </div>
                                  <span className="flex-1" />
                                  {margin > 0 && (
                                    <span className={`text-[7px] font-display font-bold ${won ? "text-neon-green/70" : "text-out-red/70"}`}>
                      ...
                      <div className="concrete-card rounded-xl p-8 text-center">
                        ...
                        {records.map((r) => {
                          ...
                          return (
                            <motion.div key={r.id} initial={{ opacity: 0, y: 5 }} animate={{ opacity: 1, y: 0 }}
                              className="concrete-card rounded-xl p-3 border border-score-gold/15">
                              <div className="flex items-center gap-3">
